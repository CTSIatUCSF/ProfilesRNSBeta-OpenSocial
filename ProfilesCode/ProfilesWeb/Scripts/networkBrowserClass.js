/*
Flash-Based Network Visualizer  
(C)opyright 2009, Harvard Medical School
All Rights Reserved.
...
network_browser = {
    ...
    Init: function(baseURL) {
        // TODO: Refactor the init/load process. inconsistent execution order between browsers causes pseudo-race condition
        if (baseURL) { this._cfg.baseURL = baseURL; }
        if (!this._swf_ref) {
    ...
    loadNetwork: function(centerID) {
        // TODO: Refactor the init/load process. inconsistent execution order between browsers causes pseudo-race condition
        if (centerID) { this.center_id = centerID; }
        if (!this._swf_ref) {
            ...
            if (!this._cfg.baseURL) {
                ...
                if (!this.loadlock) {
                    this.loadlock = true;
                    setTimeout("network_browser._swf_ref.loadNetwork(network_browser._cfg.baseURL, network_browser.center_id)", 100);
                }
                setTimeout("network_browser._loadlock=false", 3000);
            }
        }
    },
    centerOn: function(nodeID) { this._swf_ref.centerOn(nodeID); },
    clearNetwork: function() { this._swf_ref.clearNetwork(); },
    getDataRange: function(objType, objAttrib) { this._swf_ref.getDataAttributeRange(objType, objAttrib); },
    dumpEdges: function() { this._swf_ref.dumpEdges(); },
    dumpNodes: function() { this._swf_ref.dumpNodes(); },
