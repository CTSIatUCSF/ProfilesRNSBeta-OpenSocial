...
using System.Web.Caching;
    ...
    public class DataIO
    {
        ...
        {
            string sql = "select facultyranksort, max(facultyrank) facultyrank, sum(n) n from ( select (case when facultyranksort = 9 then 5 else facultyranksort end) facultyranksort, facultyrank, count(*) n  from api_query_results r with (NOLOCK), cache_person p with (NOLOCK) where r.personid = p.personid and r.queryid = '" + QueryID + "' group by facultyranksort, facultyrank ) t group by facultyranksort order by facultyranksort";
            SqlDataReader sqldr = this.GetSQLDataReader("ProfilesDB", sql, CommandType.Text, CommandBehavior.CloseConnection, null);
            return sqldr;
        }
